...
class ProfileUpdateBloc extends Bloc<ProfileUpdateEvent, ProfileUpdateState> {
  ...
  Future<void> _onInitEvent(
    ...
    emit(
      state.copyWith(
        id: event.cliente?.id,
        name: BlocFormItem(value: event.cliente?.name ?? ''),
        lastname: BlocFormItem(value: event.cliente?.lastname ?? ''),
        phone: BlocFormItem(value: event.cliente?.phone ?? ''),
        formKey: formKey,
      ),
    );
  }
  ...
  Future<void> _onUpdateUserSession(
    ...
    authResponse.cliente.name = event.cliente.name;
    authResponse.cliente.lastname = event.cliente.lastname;
    authResponse.cliente.phone = event.cliente.phone;
    authResponse.cliente.image = event.cliente.image;
  ...
  Future<void> _onFormSubmit(
    ...
    Resource response = await usersUseCases.updateUsers.run(
      state.id,
      state.toUser(),
      state.image,
      event.context,
